...
{
    public:
        SRDAfunctor (tws::matrix<TM> m, int beta) : _M( tws::matrix<TM>(m.cols(),m.cols(),0))
				{_M = (tws::transpose(m)*m); for(int i=0; i<m.cols();i++){_M(i,i) = _M(i,i)+beta;}}
				template <typename TV>        
				void operator() ( tws::vector<TV>& x, tws::vector<TV>& y) const { y = _M * x; }
    private:
				tws::matrix<TM> _M;
};
